...
`install` replaces the `mcmc.fom` function in `mcmc.py` with `fom_mean` from `noisy_mcmc.py`. After running `install()`, you can run `mcmc.mcmc_minimize()` as before, and the figure of merit should automatically be the noisy one. If you want to reaccess the noiseless figure of merit, you can run `noisy_mcmc.nominal_fom()`. The `check_limits="geometry"` ensures that any geometry which is either unbuildable or buildable yet has self-intersecting pieces, etc. doesn't get evaluated, while valid geometries that are NOT within `proposed_params_within_limits` do get built. By default, `c_cutoff` is set to `True`, unless you're working in `stability_analysis` in which it's set ot `False`. (Most likely, if you're running optimization on the noisy objective, you will have to set `c_cutoff=False` after running `install()`, i.e. run `mcmc.mcmc_minimze(..., c_cutoff=False)`.)

# 3D simulation

*NOTE*: The 3D simulation has been succesfully tested for cylindrical geometries, but it has NOT been tested for other geometries. Tests for extruded toasters are inconclusive. 